...
export default class extends Controller {
  ...
  connect() {
    ...
    inputList = document.querySelectorAll(
      '[type="text"], [type="radio"], select, textarea'
    );
    ...
    for (let i = 0; i < inputList.length; i++) {
      ...
      const error = input
        .closest(".form-row")
        .getElementsByClassName("invalid-text")[0];
  ...
  sessionTimer() {
    const sessionExpiryTimestamp =
      parseInt(this.sessionExpiryTimestampValue) * 1000;
    const sessionExpiryMaxTimestamp =
      parseInt(this.sessionExpiryMaxTimestampValue) * 1000;
    ...
    var timer = setInterval(function () {
      ...
      console.log({
        ...
        countdown:
          sessionExpiryTimestamp - parseInt(parseInt(currentDate.getTime())),
      });
      console.log({
        sessionExpiryMaxTimestamp: sessionExpiryMaxTimestamp,
        countdownMax:
          sessionExpiryMaxTimestamp - parseInt(parseInt(currentDate.getTime())),
      });
      const timeIsUp =
        sessionExpiryTimestamp <= parseInt(parseInt(currentDate.getTime()));
      const timeIsUpMax =
  ...
  checkCheckBoxes() {
    const cbRequired = document.getElementsByClassName(
      "form-row cb-required"
    )[0];
    if (cbRequired) {
      const error = cbRequired.getElementsByClassName("invalid-text")[0];
      const form_data = new FormData(document.querySelector("form"));
      if (
        !form_data.has(cbRequired.querySelector("input").getAttribute("name"))
      ) {
  ...
  toggleInputNoEmail(e) {
    const labelEmail = document.querySelector("label[for='id_email_melder']");
    const labelPhone = document.querySelector(
      "label[for='id_telefoon_melder']"
    );
    const labelEmailTextClean = labelEmail.innerHTML.split("<small>")[0];
  ...
  checkSpecifiekGraf() {
    const specifiekGrafCategorieen = JSON.parse(
      this.specifiekGrafCategorieenValue
    );
    if (specifiek_graf === 0) {
